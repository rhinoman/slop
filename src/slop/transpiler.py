...
class Transpiler:
    ...
    def __init__(self):
        ...
        self.generated_chan_types: Set[str] = set()  # Track generated Chan<T> types: (type_name, elem_c_type)
        self.generated_thread_types: Set[str] = set()  # Track generated Thread<T> types: (type_name, result_c_type)
        ...
        self.uses_concurrency: bool = False  # Track if Chan/Thread types are used (for auto-including slop_thread.h)
    ...
    def transpile(self, ast: List[SExpr]) -> str:
        ...
        # Pre-scan for Chan/Thread types to auto-include slop_thread.h
        self._prescan_for_concurrency(ast)
    ...
    def emit_forward_declaration(self, form: SList):
        ...
        mangled_name = self.to_qualified_c_name(raw_name)
        # Check for :c-name override
        c_name_override = self._extract_fn_c_name(form)
        if c_name_override:
            name = c_name_override
            # Register alias for later emission
            self.c_name_overrides[raw_name] = c_name_override
        else:
            name = mangled_name
    ...
    def emit_c_name_aliases(self):
        """Emit #define aliases for functions with :c-name"""
        if self.c_name_overrides:
            self.emit("/* Function name aliases for C interop */")
            for raw_name, clean_name in self.c_name_overrides.items():
                mangled_name = self.to_qualified_c_name(raw_name)
                if mangled_name != clean_name:
                    self.emit(f"#define {mangled_name} {clean_name}")
            self.emit("")
    ...
    def _extract_fn_c_name(self, form: SList) -> Optional[str]:
        """Extract :c-name from function form if present"""
        # Look for :c-name keyword followed by string literal in function items
        items = form.items[3:]  # Skip fn, name, params
        for i, item in enumerate(items):
            if isinstance(item, Symbol) and item.name == ':c-name':
                # Next item should be the C name string
                if i + 1 < len(items):
                    c_name_item = items[i + 1]
                    if isinstance(c_name_item, String):
    ...
    def transpile_function(self, form: SList):
        ...
        while i < len(items):
            ...
            elif isinstance(item, Symbol) and item.name == ':c-name':
                # Skip :c-name and its following value
                i += 1  # Skip the string value that follows
            else:
    ...
    def to_c_type(self, type_expr: SExpr) -> str:
        ...
        if isinstance(type_expr, SList) and len(type_expr) >= 1:
            ...
            if head == 'Thread':
                # (Thread T) -> slop_thread_{result_type}
                self.uses_concurrency = True
                inner = self.to_c_type(type_expr[1])
                inner_id = self._type_to_identifier(inner)
    ...
    def _prescan_for_concurrency(self, ast: List[SExpr]):
        """Pre-scan AST to detect if Chan or Thread types are used.

        This must be called before emitting includes so we can auto-include
        slop_thread.h when needed for concurrency primitives.
        """
        def scan_expr(expr: SExpr) -> bool:
            if isinstance(expr, SList) and len(expr) >= 1:
                head = expr[0]
                if isinstance(head, Symbol):
                    # Check for Chan or Thread type
                    if head.name in ('Chan', 'Thread'):
                        return True
                # Recursively scan children
                for item in expr.items:
                    if scan_expr(item):
    ...
    def _emit_generated_types(self, phase: str = 'all', simple_record_types: set = None):
        ...
        has_types = (self.generated_option_types or self.generated_list_types or
                     ...
                     self.generated_chan_types or self.generated_thread_types or
